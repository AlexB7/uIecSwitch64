...
; -----------------------------------------------------------
doDown:
	lda	topName	;can bottom name increment?
	clc
	adc	#MAX_LIST
	sta	a0L
	lda	topName+1
...
@90:
	rts
ckScroll:
	LoadB	r2L,LIST_T+8	;click above scroll thumb?
	ldx	thumbSav
	dex
	stx	r2H
	LoadW	r3,SCROLL_B+1
...
;	------------------------------------------------
	...
	.byte	1,7	;width (cards), height (pixels) (8px x 7px)
...
;	------------------------------------------------
	...
	.byte	48	;Y position in pixels
	.byte	6,16	;width (cards), height (pixels) (48px x 16px)
